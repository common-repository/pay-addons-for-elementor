...
class Logger
{
  ...
  public static function createLogFile()
  {
    ...
    static::$log_file =  EPA_ADDONS_LOG_FOLDER . "/{$time}.log";


    //Check if directory /logs exists
    if (!file_exists(EPA_ADDONS_LOG_FOLDER)) {
      mkdir(EPA_ADDONS_LOG_FOLDER, 0777, true);
    }
  ...
  public static  function writeLog($args = [])
  {
    ...
    if (array_key_exists('epa_sys_settings', $sys_settings) && !$sys_settings['enable_logging']) {
      return;
    }
