...
class EPA_Widget_Price_Table_Elementor extends Widget_Checkout_Base {

	public static $name = 'epa_price_table_elementor';
	...
	public function get_title() {
		return esc_html__( 'Price Table (Elementor Style)', 'elementor-pay-addons' );
	}
	...
	protected function render() {
		...
		$price = explode( $currency_format, $settings['price_amount'] );
		$intpart = $price[0];
		$fraction = '';
		if ( 2 === count( $price ) ) {
			$fraction = $price[1];
	...
	protected function content_template() {
		...
		var currencyFormat = settings.currency_format || '.',
			price = settings.price_amount.split( currencyFormat ),
			intpart = price[0],
			fraction = price[1],
